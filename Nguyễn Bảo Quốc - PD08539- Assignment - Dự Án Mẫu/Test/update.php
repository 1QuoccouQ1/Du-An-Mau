...
include 'connect.php';
// kiểm tra xem đã được submit hay chưa trong trường hợp lưu
if (isset($_POST['submit'])) {
    ...
    if ($uploadok == 0) {
        ...
        if (move_uploaded_file($_FILES["filetoUpload"]["tmp_name"], $target_file)) {
            ...
            $query = "UPDATE product SET namepro = '$name', image_url = '$path',gia = '$gia' WHERE id = '$id'";
            $result = mysqli_query($conn, $query);
            // kiểm tra kết quả try vấn
            if ($result) {
                echo "Cập nhập sản phẩm thành công";
            } else {
    ...
    if ($result) {
        ...
        $name = $row['namepro']; // lấy dữ liệu theo cột
        $gia = $row['gia']; // lấy dữ liệu theo cột
        $target_dir = $row['image_url'];
    } else {
